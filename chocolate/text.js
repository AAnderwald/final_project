...
let button = document.getElementById('messages');
let message = ["message 1", "message 2", "message 3", "message 4"];
function gogo(f){  //function can be go if individual button from fucntion go of btn1, if same js than need new name
    button.innerHTML = message[count];
    if (count == message.length -1){// has to have the -1 for the length in order to go thru all items in the array  
        count = 0 
    } 
    else{
        count++;
